...
export default function TeamPage() {
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-[21px] font-semibold text-[#1D1D1F] tracking-[-0.03em]">チームプレイ</h1>
        <p className="text-[13px] text-[#8E8E93] mt-0.5">チーム協力・バッジ・レベルの見える化</p>
      </div>

      {/* Team Members */}
      <div>
        <h3 className="text-[12px] font-semibold text-[#AEAEB2] uppercase tracking-[0.06em] mb-3 px-1">チームメンバー</h3>
        <div className="space-y-2">
          {TEAM_MEMBERS.map((m, i) => (
            <motion.div key={m.name} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="bg-white rounded-[10px] flex items-center gap-4 px-5 py-4" style={{ boxShadow: '0 0 0 1px rgba(0,0,0,0.05), 0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#FFD60A22] text-[#C07000]">Lv.{m.level}</span>
                </div>
                <div className="flex gap-1.5 mt-1">
                  {m.badges.map(b => <span key={b} className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-[rgba(0,0,0,0.04)] text-[#6E6E73]">{b}</span>)}
                </div>
              </div>
              <div className="text-right shrink-0">
                <p className="text-[12px] text-[#8E8E93]">研修実施</p>
      ...
      {/* Badges */}
      <div>
        <h3 className="text-[12px] font-semibold text-[#AEAEB2] uppercase tracking-[0.06em] mb-3 px-1">バッジ一覧</h3>
          ...
          {BADGES.map((badge, i) => {
            ...
            return (
              <motion.div key={badge.name} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.04 }}
                ...
                <p className="text-[12px] font-semibold text-[#1D1D1F] mt-2">{badge.name}</p>
                <p className="text-[10px] text-[#AEAEB2] mt-0.5">{badge.desc}</p>
              </motion.div>
            )
          })}
        </div>
