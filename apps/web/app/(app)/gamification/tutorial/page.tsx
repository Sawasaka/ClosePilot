...
export default function TutorialPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-[21px] font-semibold text-[#1D1D1F] tracking-[-0.03em]">チュートリアル</h1>
        <p className="text-[13px] text-[#8E8E93] mt-0.5">営業ロールごとのスキルロードマップとプログレス</p>
      </div>
      ...
      <div className="space-y-4">
        {TRACKS.map((track, ti) => {
          ...
          return (
            <motion.div key={track.role} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: ti * 0.06 }} className="bg-white rounded-[12px] overflow-hidden" style={{ boxShadow: CARD }}>
              <button onClick={() => setExpandedRole(isOpen ? null : track.role)} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-[rgba(0,0,0,0.015)] transition-colors">
                ...
                <div className="flex-1 text-left">
                  <p className="text-[14px] font-semibold text-[#1D1D1F]">{track.role}</p>
                  <p className="text-[12px] text-[#8E8E93] mt-0.5">{doneCount} / {track.steps.length} 完了</p>
                </div>
              ...
              <AnimatePresence>
                {isOpen && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden" style={{ borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                    <div className="px-5 py-3 space-y-1">
                      {track.steps.map((step, si) => (
                        <div key={si} className="flex items-center gap-3 py-2">
                          ...
                          <span className={`text-[13px] ${step.done ? 'text-[#AEAEB2] line-through' : 'text-[#1D1D1F]'}`}>{step.label}</span>
                        </div>
                      ))}
                    </div>
                  </motion.div>
